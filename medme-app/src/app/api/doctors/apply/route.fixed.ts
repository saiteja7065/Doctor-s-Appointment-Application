  ...
  try {
    ...
    try {
      ...
      if (existingApplication) {
        return NextResponse.json(
          {
            error: 'Doctor application already exists',
...
export async function GET() {
  try {
      ...
      return NextResponse.json(
        {
          ...
          message: 'No doctor application found for your account'
        },
        { status: 404 }
      );
    }
    ...
    return NextResponse.json({
      ...
      reviewNotes: application.adminReviews && application.adminReviews.length > 0 
        ? application.adminReviews[application.adminReviews.length - 1].notes 
