...
function getDemoData(filters: DoctorSearchFilters): DoctorSearchResult[] {
  ...
  if (filters.specialty) {
    filteredData = filteredData.filter(doctor => doctor.specialty === filters.specialty);
  }

  if (filters.minRating) {
    filteredData = filteredData.filter(doctor => doctor.rating >= filters.minRating!);
  }

  if (filters.maxFee) {
    filteredData = filteredData.filter(doctor => doctor.consultationFee <= filters.maxFee!);
  }
  ...
  if (filters.search) {
    ...
    filteredData = filteredData.filter(doctor =>
      ...
      doctor.formattedSpecialty.toLowerCase().includes(searchLower)
    );
  }
